...
def test_build_tree_with_combined_limits():
    ...
    depth_truncations = sum(
        1
        for n in truncation_nodes
        if "level_1_folder" in n.parent and n.parent.count("/") == 1
    )
    ...
    print(
        f"Total truncations: {total_truncations} (depth: {depth_truncations}, nested: {nested_truncations}, global: {global_truncations})"
    )
...
def test_exact_truncation_scenario():
    ...
    print(
        f"Depth 1 test: {len(truncated_tree)} total nodes, {len(truncation_nodes)} truncation indicators"
    )
